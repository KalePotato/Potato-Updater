...
Potato Updater 是一个用于 Minecraft/PCL 客户端的启动前更新工具。它通过 Java Agent 在游戏加载前运行更新器，根据远端清单检查并同步模组、资源包、光影包和启动器文件。
...
Potato Updater is a pre-launch updater for Minecraft clients using PCL. It runs through a Java Agent before the game loads, then checks a remote manifest and synchronizes managed mods, resource packs, shader packs, and launcher files.
